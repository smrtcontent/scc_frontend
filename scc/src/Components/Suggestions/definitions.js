...
const Definitions = (props) => {
   ...
   */
  const meaning = () =>
    props.meanings.map((x) =>
      x.meanings.map((x) =>
        x.definitions.map((x) => definitions.push(x.definition))
      )
    );
  ...
  if (props.meanings.length > 0) {
    // This will check if the props contain definition or not
    // This section will be returned only if the props contain definitions
    return (
      <div id="scroll-blue" className="suggestions ps-2">
